...
public class LevelWonLayer extends ManagedLayer {
	...
	public void onLoadLayer() {
		...
		final GrowButton BackToLevelSelectButton = new GrowButton(128.2f, 75f, ResourceManager.gameLevelLayerButtonLevelSelectTR, false) {
			@Override
			public void onClick() {
		...
		final GrowButton RestartLevelButton = new GrowButton(256f, 75f, ResourceManager.gameLevelLayerButtonRestartLevelTR, false) {
			@Override
			public void onClick() {
		...
		final GrowButton NextLevelButton = new GrowButton(383f, 75f, ResourceManager.gameLevelLayerButtonNextLevelTR, false) {
			@Override
			public void onClick() {
				LevelWonLayer.this.mIsGoingToRestartLevel = false;
	...
	public void onShowLayer() {
		...
		final int PreviousHighScore = TimelessWarriorsActivity.getIntFromSharedPreferences(TimelessWarriorsActivity.SHARED_PREFS_LEVEL_HIGHSCORE + this.mCurrentLevel.mLevelDef.mLevelIndex);
		if(PreviousHighScore >= this.mCurrentLevel.CurrentScore) {
			this.mHighScoreText.setText("HIGHSCORE: " + PreviousHighScore);
		} else {
			TimelessWarriorsActivity.writeIntToSharedPreferences(TimelessWarriorsActivity.SHARED_PREFS_LEVEL_HIGHSCORE + this.mCurrentLevel.mLevelDef.mLevelIndex, this.mCurrentLevel.CurrentScore);
		...
		if(NumStars > TimelessWarriorsActivity.getIntFromSharedPreferences(TimelessWarriorsActivity.SHARED_PREFS_LEVEL_STARS + this.mCurrentLevel.mLevelDef.mLevelIndex)) {
			TimelessWarriorsActivity.writeIntToSharedPreferences(TimelessWarriorsActivity.SHARED_PREFS_LEVEL_STARS + this.mCurrentLevel.mLevelDef.mLevelIndex, NumStars);
		}
		...
		final int currentMaxLevel = TimelessWarriorsActivity.getIntFromSharedPreferences(TimelessWarriorsActivity.SHARED_PREFS_LEVEL_MAX_REACHED);
		if(currentMaxLevel < this.mCurrentLevel.mLevelDef.mLevelIndex) {
			TimelessWarriorsActivity.writeIntToSharedPreferences(TimelessWarriorsActivity.SHARED_PREFS_LEVEL_MAX_REACHED, this.mCurrentLevel.mLevelDef.mLevelIndex);
		}
	}
