...
public class ResourceManager extends Object {
	...
	public TimelessWarriorsActivity activity;
	...
	public static TiledTextureRegion Hero_Die;
	public static TiledTextureRegion Hero_Move;

	public static TextureRegion gameMetalBeamDynamicTR;
	public static TextureRegion gameMetalBeamStaticTR;
	public static TextureRegion gameCoinDynamicTR;

	public static ITiledTextureRegion bullet_region;
	...
	public static void setup(TimelessWarriorsActivity pActivity,
	...
	public static void loadGameResources() {
		...
		QuestionsLayer.getInstance().onLoadLayer();
		LevelLoseLayer.getInstance().onLoadLayer();
	}
	...
	public static TimelessWarriorsActivity getActivity() {
		return getInstance().activity;
	}

	public static TimelessWarriorsSmoothCamera getCamera() {
		return (TimelessWarriorsSmoothCamera) getInstance().engine.getCamera();
		...
		activity.runOnUpdateThread(new Runnable() {
			...
			public void run() {
				TimelessWarriorsActivity
						.writeBooleanToSharedPreferences(
								TimelessWarriorsActivity.SHARED_PREFS_HIGH_QUALITY_GRAPHICS,
	...
	private void loadGameTextures() {
		...
		mPreviousAssetBasePath = BitmapTextureAtlasTextureRegionFactory
				.getAssetBasePath();
		BitmapTextureAtlasTextureRegionFactory.setAssetBasePath("Sprite/Hero/");
		...
		if (Hero_Move == null) {
			Hero_Move = getLimitableTTR("Hero_Move.png", 6, 1,
					mTransparentTextureOption);
		}
		if (Hero_Die == null) {
			Hero_Die = getLimitableTTR("Hero_Die.png", 5, 4,
					mTransparentTextureOption);
		}
		if (bullet_region == null) {
			bullet_region = getLimitableTTR("skill.png", 6, 3,
					mTransparentTextureOption);
		}
		

		/*
		 * if (Hero_Attack1 == null) { Hero_Attack1 =
		 * getLimitableTTR("Hero_Attack1.png", 2, 8, mTransparentTextureOption);
		 * } if (Hero_Attack1_Ball == null) { Hero_Attack1_Ball =
		 * getLimitableTTR("Hero_Attack1_Ball.png", 1, 4,
		 * mTransparentTextureOption); } if (Hero_Attack1_Hit == null) {
		 * Hero_Attack1_Hit = getLimitableTTR("Hero_Attack1_Hit.png", 6, 1,
		 * mTransparentTextureOption); }
		 */
		...
		if (gameCrateSmallTTR == null)
			gameCrateSmallTTR = getLimitableTTR("CratesSmall.png", 2, 2,
					mTransparentTextureOption);
		if (gameCoinDynamicTR == null) {
			BitmapTextureAtlas metalBeamDynamicT = new BitmapTextureAtlas(
					textureManager, 128, 128, mStretchableBeamTextureOption);
			gameCoinDynamicTR = BitmapTextureAtlasTextureRegionFactory
					.createFromAsset(metalBeamDynamicT,
							ResourceManager.getActivity(), "coin.png", 0, 0);
		...
		// Load the textures of the game level that will not require
		// stretching
		if (gameMetalBeamDynamicTR == null) {
			BitmapTextureAtlas metalBeamDynamicT = new BitmapTextureAtlas(
					textureManager, 212, 64, mStretchableBeamTextureOption);
	...
	private void loadMenuTextures() {
		...
		mPreviousAssetBasePath = BitmapTextureAtlasTextureRegionFactory
				.getAssetBasePath();
		BitmapTextureAtlasTextureRegionFactory
				.setAssetBasePath("MagneTank/Menu/");

		if (menuLevelIconTR == null) {
			BitmapTextureAtlas LevelIconT = new BitmapTextureAtlas(
					ResourceManager.getActivity().getTextureManager(), 64, 64,
					mTransparentTextureOption);
			menuLevelIconTR = BitmapTextureAtlasTextureRegionFactory
					.createFromAsset(LevelIconT, ResourceManager.getActivity(),
							"LevelIcon.png", 0, 0);
			LevelIconT.load();
		}
		...
		if (menuLevelLockedTR == null) {
			BitmapTextureAtlas LevelLockedT = new BitmapTextureAtlas(
					ResourceManager.getActivity().getTextureManager(), 64, 64,
					mTransparentTextureOption);
			menuLevelLockedTR = BitmapTextureAtlasTextureRegionFactory
					.createFromAsset(LevelLockedT,
							ResourceManager.getActivity(), "Lock.png", 0, 0);
			LevelLockedT.load();
		}

		if (menuLevelStarTTR == null) {
			BitmapTextureAtlas LevelStarT = new BitmapTextureAtlas(
					ResourceManager.getActivity().getTextureManager(), 64, 64,
					mTransparentTextureOption);
			menuLevelStarTTR = BitmapTextureAtlasTextureRegionFactory
					.createTiledFromAsset(LevelStarT,
							ResourceManager.getActivity(), "Stars.png", 0, 0,
							4, 1);
			LevelStarT.load();
		}

		if (menuArrow1TR == null) {
			BitmapTextureAtlas arrow1T = new BitmapTextureAtlas(ResourceManager
					.getActivity().getTextureManager(), 64, 64,
					mTransparentTextureOption);
			menuArrow1TR = BitmapTextureAtlasTextureRegionFactory
					.createFromAsset(arrow1T, ResourceManager.getActivity(),
							"Arrow1.png", 0, 0);
			arrow1T.load();
		}

		if (menuArrow2TR == null) {
			BitmapTextureAtlas arrow2T = new BitmapTextureAtlas(ResourceManager
					.getActivity().getTextureManager(), 64, 64,
					mTransparentTextureOption);
			menuArrow2TR = BitmapTextureAtlasTextureRegionFactory
					.createFromAsset(arrow2T, ResourceManager.getActivity(),
							"Arrow2.png", 0, 0);
			arrow2T.load();
		}

		if (menuBackgroundTR == null)
			menuBackgroundTR = getLimitableTR("BG.png", mNormalTextureOption);
		if (menuMainTitleTR == null)
			menuMainTitleTR = getLimitableTR("MagneTankTitle.png",
					mTransparentTextureOption);
		if (menuMainButtonsTTR == null)
			menuMainButtonsTTR = getLimitableTTR("MainMenuButtons.png", 1, 4,
					mTransparentTextureOption);
	...
	private void loadSharedTextures() {
		...
		if (MusicToggleTTR == null)
			MusicToggleTTR = getLimitableTTR("MusicToggle.png", 2, 1,
					mTransparentTextureOption);

		if (SoundToggleTTR == null)
			SoundToggleTTR = getLimitableTTR("SoundToggle.png", 2, 1,
					mTransparentTextureOption);
	...
	private void loadFonts() {
		...
		if (fontDefaultMagneTank48 == null) {
			fontDefaultMagneTank48 = getFont(Typeface.createFromAsset(
	...
	private void updateTextBounds(final Typeface pTypeface, final float pSize,
			final boolean pAntiAlias, final String pCharacterAsString) {
		mPaint = new Paint();
		mPaint.setTypeface(pTypeface);
		mPaint.setTextSize(pSize);
		mPaint.setAntiAlias(pAntiAlias);
		mPaint.getTextBounds(pCharacterAsString, 0,
				pCharacterAsString.length(), this.mTextBounds);
	}
}