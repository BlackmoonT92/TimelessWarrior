...
public class LevelSelector extends Entity {
	...
	/*
	 * ff mChapter: This value defines the LevelSelector class' chapter value.
	 * This variable can allow us to create a number of LevelSelector objects
	 * which represent different chapters/worlds/zones within our game by
	 * specifying different chapter values for each LevelSelector object
	 */
	...
	public LevelSelector(final float pX, final float pY,
			final int maxCompletedLevel, final int pWorldIndex,
		...
		//this.mNumLevelsInWorld = 10;
		// Set the initial position in order to properly center the tiles
		this.mInitialX = -(((mTILE_DIMENSION + mTILE_PADDING) * mCOLUMNS) / 2f)
				+ (mTILE_DIMENSION + mTILE_PADDING) / 2f;
		this.mInitialY = (((mTILE_DIMENSION + mTILE_PADDING) * mROWS) / 2f)
				- (mTILE_DIMENSION + mTILE_PADDING) / 2f;
	...
	private void createTiles(final ITextureRegion textureRegion, final Font font) {
		...
		for (int i = 0; i < mROWS; i++) {
			...
			for (int o = 0; o < mCOLUMNS; o++) {
				...
				final LevelSelectorButton levelButton = new LevelSelectorButton(
						currentTileLevel, mWorldIndex, tempX, tempY,
						mTILE_DIMENSION, mTILE_DIMENSION, textureRegion,
