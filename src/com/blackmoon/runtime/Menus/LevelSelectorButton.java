...
/**
 * The LevelSelectorButton class visually shows the player the state of a level,
 * locked or unlocked, and the number of stars achieved if the level is
 * unlocked.
 * 
 **/
public class LevelSelectorButton extends Sprite {
	...
	// CONSTRUCTOR
	// ====================================================
	public LevelSelectorButton(final int pLevelIndex, final int pWorldIndex,
		...
		this.mButtonText = new Text(0f, 0f, ResourceManager.fontDefault32Bold,
				String.valueOf(this.mLevelIndex), ResourceManager.getActivity()
		...
		this.mLockedSprite = new Sprite(this.getWidth() / 2f,
				this.getHeight() / 2f, ResourceManager.menuLevelLockedTR,
				ResourceManager.getActivity().getVertexBufferObjectManager());

		this.mStarsEnt = new TiledSprite((this.getWidth() / 3f) * 2f,
				(this.getHeight() / 2f), ResourceManager.menuLevelStarTTR,
				ResourceManager.getActivity().getVertexBufferObjectManager());

		this.mIsLocked = (this.mLevelIndex > (TimelessWarriorsActivity
	...
	protected void onManagedUpdate(final float pSecondsElapsed) {
		...
		if (!this.mIsLocked) {
			if (!this.mButtonText.hasParent()) {
				...
				this.mStarsEnt.setCurrentTileIndex(TimelessWarriorsActivity
						.getLevelStars(this.mLevelIndex));
			}
				...
				this.registerEntityModifier(new ScaleModifier(0.05f, 1.4f, 1f) {
					...
					protected void onModifierFinished(final IEntity pItem) {
						...
						if (LevelSelectorButton.this.mIsClicked) {
							Log.d("<<LEVEL SELECTOR>>", "CLICKED LEVEL");
							/*GameLevel gl = new GameLevel(new LevelDef(1, 1));
							SceneManager.getInstance().showScene(gl);*/
